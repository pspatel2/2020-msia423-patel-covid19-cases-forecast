...
def get_country_daily(df):
    ...
    '''
    df.drop(columns=["Province","City","CityCode","Lat","Lon"])
    return df
...
def run_data_preparation(args):
    ...
    Args:
        args: From argparse:
           ...
           - RDS (bool): the flag used to determine where the database schema will be created (RDS vs Local)
    ...
    helper.add_to_database(country_df,"country_covid_daily_cases",'replace',args.RDS,args.engine_string)
    helper.add_to_database(global_df, "global_covid_daily_cases",'replace',args.RDS, args.engine_string)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--RDS", dest='RDS',action='store_true',help="Use arg if if you want to create in RDS rather than in local database.")
